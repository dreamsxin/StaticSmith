...
**锁真被污染了也不会把窗口变成废的。** `catch_unwind` 只覆盖闭包本身，而持锁期间还有
别的代码在跑（`with_writing_session` 里留快照那一步），文件监听线程更完全不在覆盖范围内。
所以取锁一律走 `lock_or_recover`：污染时取出里面的值继续用，并留一条警告日志。
理由是「污染的含义是上次持锁时有人 panic 了，不是里面的数据坏了」——`Option<Session>`
的一致性由写入路径的重开项目负责，自身写入表里只是一串带 TTL 的路径。
跟着 panic 的代价则完全不对称：项目还开着，但每个命令都在取锁那一行炸，
用户只能重启并丢掉未保存的东西。

后台线程（预览 HTTP、文件监听）的循环体各自套了 `staticsmith_core::util::keep_running`：
一次 panic 只让**这一轮**作废（一个请求不回、一批变更不通知），线程接着跑下一轮。
这两处以前是「panic 就结束线程」，表现为「预览突然打不开了」「外部改动再也不提示了」
这类静默失效——端口还在监听、界面还写着运行中，唯一线索是 panic 钩子写的日志。
内嵌 MCP 的**工具调用**自己兜住了（见 [MCP 服务](mcp.md)）：一次 panic 变成一条
JSON-RPC 错误，端点继续活着。
...
还含 `broken_sources: {source, reason}[]`——**读不出来的源文件**（多半是 front matter
手改坏了）。这几篇不在 `pages` 里，但项目照样打得开：编辑器就是用来修它的，
以前 `Builder::open` 直接失败，一个坏文件能让整站编辑不了。前端必须把它报出来
（`store` 里按原因汇总弹一条，明细进消息中心），否则用户只看到列表里凭空少一篇。
`build_site` 会在这种状态下拒绝生成——产物里静默少一页比报错难查得多。


## 配置

- `read_config() -> SiteConfig`
- `save_config(config) -> string[]`：校验失败直接报错，不写盘；成功后重开 Builder
