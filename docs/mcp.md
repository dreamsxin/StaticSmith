...
- `rename_section`：栏目改名。默认 `keep_aliases = true`，给每篇文章补旧地址，
  ...
  （整栏目换名等于一批地址同时变）。带 `dry_run: true` 只回传
  「会搬几个文件、补几个旧地址、改写哪几篇里的几处引用」
...
- `move_content`：把一篇内容搬到另一个栏目，默认补旧地址。`to_section` 传空串
  ...
  搬动会把指向这一篇的地址改到新位置，Agent 接下来若要编辑那几篇，得先重新读一遍
...
签出范围是**当前**跟踪清单与**目标快照那一刻**跟踪清单的并集。只用当前那套的话，
跨过一次目录改名的回退会两头落空：旧目录在 pathspec 之外一个字节都不恢复，
新目录在 pathspec 之内且目标树里没有于是被删干净，而 `staticsmith.toml` 会被换回旧版——
配置指向一个空目录，整站内容消失，却报「回退成功」。


桌面端也共用这条历史：界面上的删除、批量动作（改标签 / 改草稿 / 搬动 / 删除）、
栏目改名与删除、跨文件替换、内容导入、主题装入、配置保存（表单与源码两处）、
删除媒体，在动手前同样会留快照，Agent 与人看到的是同一份历史。

**故意不留快照的是「有缓冲兜着」的那几个**：单篇保存、保存模板、新建文章 / 栏目、
改栏目信息。它们改的是编辑器里正打开的那一份，撤销靠的是编辑器自己的缓冲；
每次 Ctrl+S 都留一份快照只会把历史冲成噪声，真要回退的那一笔反而更难找。

界面上的入口是「编辑 → 回退内容…」，列出的是同一批快照，包括 Agent 留下的那些。
...
- **一次崩溃不会让端点停止服务**。工具执行外面套了 `catch_unwind`：程序缺陷导致的
  panic 变成一条 JSON-RPC 内部错误（Agent 读得到原因），而不是掀掉 HTTP 工作线程
  或 stdio 主循环——那种情况下端点静默失效，界面上还写着「运行中」。
  兜住之后会立刻从磁盘重载项目状态：panic 可能停在改了一半的地方。
  状态锁被上一次 panic 污染时也一样恢复着用，不会从此每个请求都失败。
  HTTP 那条路还多兜一层：**请求分派**（路由判断、读请求体、拼 JSON-RPC、查会话表）
  出 panic 时回 500 并说明「已兜住，端点仍在运行」，而不是让这个请求永远不回话
  ——客户端只能等到超时，日志里也没有线索。请求线程外面还有最后一道，
  管的是发响应与 SSE 长流那一段。
