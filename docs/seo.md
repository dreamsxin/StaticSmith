...
死链最常见的来源是自己改了地址。这种情况别只改链接——在文章的 front matter 里写
`aliases = ["/posts/old-slug/"]`，构建会为旧地址生成重定向页（`meta refresh` +
指向新地址的 `canonical` + `noindex`），站外已经存在的老链接与搜索结果都还能用。
属性面板有「旧地址（重定向）」一栏，Agent 侧对应 `patch_front_matter` 的 `aliases`。
