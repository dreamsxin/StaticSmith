...
**指向站内却不存在的地址会在生成报告里报出来**（`staticsmith_core::menu`）：菜单渲染在
每一页的头部，所以它是唯一「一处写错、全站死链」的配置，而死链原先只能等体检发现。
比较时末尾斜杠与锚点不算差异（产物里 `/about` 与 `/about/` 是同一个文件），
站外链接与纯锚点不查——它们的正确性不在这个站里。
**只警告不阻断**：「先占个位、页面等会儿建」是合法用法。设置界面在编辑时就会提示同样的
问题（那一份在 `src/menu.ts`，与 core 的规则是孪生的）。
...
- **阻断**（`SiteConfig::validate`）：`site.title` 为空、`page_size` 为 0、
  选了 `deploy.type` 却缺配置段、两个分类维度读同一个字段、`sftp = true` 等。
  这些会让项目**打不开**，界面上保存也会被拒绝。
- **提醒**（`SiteConfig::warnings`）：合法但有代价的配置。不阻断任何事，
  在设置页顶部显示，`staticsmith check` 里以「注意：」开头打印，日志里也有一份。

分两层的理由：把提醒混进阻断，会让一直正常工作的老项目升级后突然打不开，
那个代价比「没提醒」大得多。

目前只有一条提醒：**源目录走出了项目根**（`content_dir` / `template_dir` /
`theme_dir` / `static_dir` 写成了绝对路径或 `../`）。它合法，但有两个不显眼的后果：

- 内容快照按项目根算跟踪范围，走出去的目录**不进快照**——安全网静默变小了
- `resolve_source` 那套「路径必须在根之内」的校验失去了参照

`output_dir` 指到项目外**不提醒**：产物目录本来就可以随时重新生成，
指向站外是常规用法（等价于 `hugo -d /var/www`）。

## 路径校验：字面 + 解析两道

所有来自界面、Agent 与主题包的路径都过两道判断：

1. **字面上**挡掉 `..`、绝对路径、盘符（`D:foo`）、以反斜杠开头的路径与系统保留名。
   待写入的文件常常还不存在，这一步不碰磁盘。
2. **解析符号链接**之后再确认一次仍在目标目录内（`util::is_within_resolved`）。
   办法是从目标往上找到第一个已存在的祖先、规范化它，再把剩下那截按字面接回去比对——
   安全解压器与静态文件服务器都是这个思路。

第二道是必需的：`content/外链` 指向别处时，`外链/x.md` 在**字面上**完全合法，
而 `fs::write` 会顺着链接写到目录外面去。内容、栏目、主题包与媒体删除四处共用同一份
判断，各写一份的话规则改一处漏三处，而这是安全判断。

## 文件名不能用系统保留名

`nul`、`con`、`prn`、`aux`、`com1`–`com9`、`lpt1`–`lpt9` 在 Windows 上不是文件而是
**设备**：写进去「成功」了，内容却进虚空、读回来是空的，而且不报任何错。
保留名对任何扩展名都生效（`nul.md` 一样是 NUL）。

所以新建内容、新建栏目、装主题包时这些名字一律被拒绝，**三个平台一致**——
在 Linux 上建出 `nul.md`，拿到 Windows 就是打不开的一篇，把差异留到那时才暴露最糟。
只是「包含」保留名的普通名字不受影响（`console.md`、`com10.md` 都正常）。
...
删除（仅空栏目）。文章与栏目的位次都用右键菜单里的「上移 / 下移」调，第一次挪动会把
整栏 / 整层的顺序固化成 `weight = 1..N`（界面上不出现这个数字）。排栏目顺序时，
缺索引页的栏目会顺手补一张——位次只能存在那里。Agent 侧对应 `list_sections` /
