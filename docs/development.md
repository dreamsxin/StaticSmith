...
- **组件测试用 `@vue/test-utils` + jsdom**，但**默认环境仍是 node**：要 DOM 的文件
  ...
  （大纲、站内链接）、编辑器的「改地址」、发布面板的「发布确认」与侧栏的「栏目头」。
  挑这几处是因为它们是「焦点归还、Esc、方向键、就地确认后剩什么状态、没确认不许写」
  的落地处，而这几条约定（docs/ui.md）以前只能靠人工推理——第一次跑就抓到一个真 bug：
  换文章后待确认的改地址没有作废。
- **测不动往往是拆分的信号**：栏目管理原先埋在 `PageList.vue`（1298 行）里，
  为了给它写测试才抽成 `SectionHeader.vue`；抽完立刻能钉住改名确认态的四个出口。
  拆的边界按「与其余部分有没有共享状态」来选，而不是按行数：栏目那块不碰筛选、
  选中集合与搜索结果，所以是最干净的一刀。
