...
- **组件测试用 `@vue/test-utils` + jsdom**，但**默认环境仍是 node**：要 DOM 的文件
  ...
  与「跨文件替换」。挑这几处是因为它们是「焦点归还、Esc、方向键、就地确认后剩什么状态、
  没确认不许写」的落地处，而这几条约定（docs/ui.md）以前只能靠人工推理——
  第一次跑就抓到一个真 bug：换文章后待确认的改地址没有作废。
- **`store.ts` 的忙态计数器有测试了**（`src/store.test.ts`）：`store.busy` 由计数器
  而非布尔驱动，一次复合动作（`build` 内部顺序调用四五次 `run`）只许亮一次。
  这条性质此前只写在注释里，而它是所有「按钮该不该置灰」的地基。
  测法：`vi.mock('./api')` 按真模块的键批量换成假实现（不手写清单，加新 api 不会漏），
  改行为只改「这次返回什么」的映射表——**不要替换映射表里的函数**，
  `store` 在导入时就抓住了那几个引用，换掉它看不见。
- **测不动往往是拆分的信号**：栏目管理与跨文件替换原先埋在 `PageList.vue`（1298 行）里，
  为了给它们写测试才抽成 `SectionHeader.vue` 与 `ReplacePanel.vue`（现在 850 行）。
