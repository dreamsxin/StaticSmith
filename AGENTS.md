...
- **样式表是按 `@import` 顺序拼起来的**：`src/styles.css` 只有一串 `@import`，规则在
  ...
  所以护栏管的是最终生效的那份。
- **改文件时的锚点必须是文件里唯一的字符串**，而且**必须先确认它此刻还在**。
  这个仓库的注释密度很高，拿 `/**`、`function` 这类到处都有的片段当锚点，
  会一次改掉几十处（真发生过：`store.ts` 从 1604 行涨到 4935 行）。
  另一种更隐蔽的死法是**跨轮复用上次用过的锚点**：`CHANGELOG.md` 的「未发布」段
  上一轮多了个「### 新增」小节，而我照抄了上一轮的 `## 未发布\n\n### 修复`——
  那个组合已经不存在了，替换却"成功"了，代价是未发布段里 200 行历史条目被吞掉
  （靠 `git checkout <上一个提交> -- CHANGELOG.md` 救回）。**改之前先读那一段**。
  动 `store.ts`、`styles/*.css`、`PageList.vue`、`CHANGELOG.md` 这几个大文件尤其要先读。
