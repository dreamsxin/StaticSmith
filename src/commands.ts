...
/**
 * 新建站点：收到表单填写的名称与版式后选目录并写入。
 *
 * 名称与版式由 `NewSiteDialog` / 起始页的卡片收集（两处共用 `NewSiteForm`）。
 * 版式必须让人选：它推导不出来，事后换代价也不小（一套是十来个模板文件，
 * 换等于重新脚手架）。
 */
export async function createSiteWith(title: string, preset: string) {
  const path = await pickDirectory()
  if (!path) return
